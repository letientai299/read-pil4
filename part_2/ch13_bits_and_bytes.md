...
> Implement different ways to compute the number of bits in the representation
> of a Lua integer.
...
> Write a function to compute the Hamming weight of a given integer. (The
> Hamming weight of a number is the number of ones in its binary
> representation.)

### Exercise 13.5

> Write a function to test whether the binary representation of a given integer
> is a palindrome.
...
> Suppose we have a binary file with a sequence of records, each one with the
> following format:
>
> ```c
> struct Record {
